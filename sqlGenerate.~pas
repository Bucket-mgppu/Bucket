...
var
  ...
  ArrSourceClass: array [1..20] of AnsiString; // ВВОД: список исходных классов, возможных в источниках
 ...
 // ArrDisjointClasses: array [1..6,1..6] of integer; // Таблица отношений классов - бинарные джойнт и дисджойнт
  ArrDisjointClasses: array of array of integer ; // что бы иметь возможность добавлять классы я желаю массив динамическим   (Кирилл)
...
//SourceBucketRating: array [1..100] of integer; // Ретинг Источников для сортировки и выполнения в порядке понижения рейтинга
  ...
  kon : integer;  // хранит номер последнего непустого эл-та в массиве AeeSourceClasses
  najal : integer; // нужна для управления процессом добавления классов
  najal2raza : integer; // что бы нельзя было добавить два новых столбца
...
// Генерирование SQL запроса
   // ---НАЧАЛО СБОРА ЦЕЛЕВЫХ ПОЛЕЙ ArrQueryWhat[]---
      ...
      begin
        ...
        begin
          // при старом интерфейсе if (FindComponent(ArrSourceCapabilities[q]) as TCheckBox).Checked = true
   ...
   // ---НАЧАЛО ПОИСКА StrQuerySearches---
      ...
      for i:=1 to count do begin
        // при старом интерфейсе было if ((FindComponent('Search' + ArrQuerySearches[i]) as TEdit).Text <> '') AND ((FindComponent(ArrQuerySearches[i]) as TCheckBox).Checked = true)
...
(* Старый интерфейс if Category.Checked = false
...
// ВВОД: ОПИСАНИЕ ВХОДНЫХ ДАННЫХ
      ...
      While not Eof(F) do begin
        ReadLn(F,ArrSourceClass[indmas]);
        Inc(indmas);
      end;
      CloseFile(F);
 ...
 StringGrid1.RowCount:= StrToInt(rowtabl);
 StringGrid1.ColCount:= StrToInt(coltabl);
      ...
      While not Eof(D) do begin
        for i:=0 to High(ArrDisjointClasses) do begin
           ...
           for j:=0 to (Length(tablstr))-1 do begin
               ArrDisjointClasses[i,j] := StrToInt(tablstr[j+1]);
               StringGrid1.Cells[j+1,i+1] := tablstr[j+1];
           end;
           tablstr:='';
        end;
      end;
    ...
    {  for j:=1 to high (ArrSourceClass) do
      begin                                       // убрал эту часть и заменил кодом  ниже   (Кирилл)
...
// ОПИСАНИЕ КЛАССОВ SourceClassesCapabilities[classname][i] = capability
  ...
  for i:=1 to (StringGrid1.RowCount - 1) do begin
    if ArrSourceClass[i]<>'' then
      StringGrid1.Cells[0,i]:= ArrSourceClass[i]
    else Break;
  end;

  for i:=1 to (StringGrid1.ColCount - 1) do begin
    if ArrSourceClass[i]<>'' then
      StringGrid1.Cells[i,0]:= ArrSourceClass[i]
    else Break;
  end;
...
// Проверяем источники на джойнт-дисдойнт
      ...
      x := 1;
                          ...
                          // Пусть пользователь удивится! Здесь надо ставить Exit; - дальше выполнять незачем.
                    ...
                    begin
                       // TestMemo2.Lines.Add(IntToStr(srs) + ' источник сработал (пред-inclass цикл)');
                        ...
                        begin
                          ...
                          // Как бы по-умолчанию все источники подходят по ограничениям, а потом, если сработают ограничения, УБЬЕМ ИСТОЧНИКИ!
                          RestrictOrNot[srs] := 1;
...
// Вывод списка джойнед-источников, умножаем со значениями из ограничений.
 SrsCount := 0;
 TestMemo2.Lines.Add('### Sources selected by disjoint matrix ###');
 for j:=1 to high(DjointOrNot) do   // по всем классам в запросе
 begin
...
// Вывод списка отобранных источников в Ведро на основе отношений классов и ограничений в запросе глупого юзера
 SrsCount := 0;
 TestMemo2.Lines.Add('### Sources selected by user restrictions ###');
 for j:=1 to high(Bucket) do   // по всем классам в запросе
 begin
      // надо ограничить условие, а то гоняет 100 раз!
      if (Bucket[j] > 0) then   // если элемент массива не пустой
      begin
...
begin
 ...
 if Edit2.Text='' then begin
   Showmessage('Введите имя нового класса');
   Exit;
 end;

 if najal=0 then begin
   Showmessage('Добавьте новые строку и столбец');
   Exit;
 end;
 najal:=1;
 for i:=1 to (StringGrid1.ColCount)-2 do begin
    if StringGrid1.Cells[(StringGrid1.ColCount)-1,i]='' then begin
    Showmessage('Введите отношения классов');
    Exit;
    end;
 end;
 for i:=1 to High(ArrSourceClass) do begin
   if ArrSourceClass[i]='' then begin
     ArrSourceClass[i]:=Trim(Edit2.Text);
     StringGrid1.Cells[StringGrid1.ColCount-1,0]:= ArrSourceClass[i];
     StringGrid1.Cells[0,StringGrid1.RowCount-1]:= ArrSourceClass[i];
     Break;
   end;
 end;
 ...
 CloseFile(F);
   ...
   for i:=1 to (StringGrid1.ColCount)-1 do begin
      StringGrid1.Cells[i,(StringGrid1.ColCount)-1]:=StringGrid1.Cells[(StringGrid1.ColCount)-1,i];
   end;
 ...
 tablstr:='';
   for j:=1 to StringGrid1.ColCount-1 do begin
      tablstr:=tablstr + StringGrid1.Cells[j,i];
      ArrDisjointClasses[i-1,j-1] :=StrToInt(StringGrid1.Cells[j,i]);
   end;
 WriteLn(D,tablstr);  
 end;
 CloseFile(D);

 Showmessage('Новый класс успешно добавлен');
...
SetLength( ArrDisjointClasses , StringGrid1.RowCount-1 , StringGrid1.ColCount-1 );
